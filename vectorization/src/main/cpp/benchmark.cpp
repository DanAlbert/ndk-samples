...
#error check if std::simd works yet
    ...
    case Backend::kNeon:
      ...
      LOG(INFO) << "Benchmarking Neon";
      return Benchmark(position, translation,
                       [](Vec4 p, Mat4 t) { return MultiplyWithNeon(t, p); });
#else
      LOG(INFO) << "Neon is not supported on this device";
      return std::unexpected{BenchmarkError::kNotSupported};
#endif
