...
void InitLogging(const std::optional<std::string_view> default_tag = {}, std::optional<LogSeverity> log_level = {},
...
#define WOULD_LOG(severity)                                                              \
  (UNLIKELY(::ndksamples::base::ShouldLog(SEVERITY_LAMBDA(severity), _LOG_TAG_INTERNAL)) || \
...
#define LOG_STREAM(severity)                                                                    \
  ::ndksamples::base::LogMessage(__FILE__, __LINE__, SEVERITY_LAMBDA(severity), _LOG_TAG_INTERNAL, \
                              -1)                                                               \
      .stream()
...
#define LOGGING_PREAMBLE(severity)                                                         \
  (WOULD_LOG(severity) &&                                                                  \
   ABORT_AFTER_LOG_EXPR_IF((SEVERITY_LAMBDA(severity)) == ::ndksamples::base::FATAL, true) && \
...
#define PLOG(severity)                                                           \
  LOGGING_PREAMBLE(severity) &&                                                  \
      ::ndksamples::base::LogMessage(__FILE__, __LINE__, SEVERITY_LAMBDA(severity), \
...
#define CHECK(x)                                                                                 \
  LIKELY((x)) || ABORT_AFTER_LOG_FATAL_EXPR(false) ||                                            \
      ::ndksamples::base::LogMessage(__FILE__, __LINE__, ::ndksamples::base::FATAL, _LOG_TAG_INTERNAL, \
                                  -1)                                                            \
              .stream()                                                                          \
          << "Check failed: " #x << " "
